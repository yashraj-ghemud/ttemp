Act as a Senior C# Developer specializing in DirectX/ImGui rendering and Game Engine Memory Analysis for Counter-Strike  (CS 1.6). 
...
Please implement the following modules for CS:

1. MEMORY ACCESS & OFFSET MANAGEMENT:
- Provide a template for a 'Memory' class that uses OpenProcess/ReadProcessMemory to access the ca client.dll and engine2.dll modules.
- Include logic to fetch the 'LocalPlayer' pointer and the 'EntityList'.

2. WORLD-TO-SCREEN PROJECTION:
- Write the C# function to calculate 3D-to-2D coordinates using the ca ViewMatrix. This is required to draw ESP boxes on the screen.
...
Please provide the complete C# class structure, and include the necessary Win32 API imports needed for this external overlay to run on top of the CS window.
